...
void loop() {
  ...
  switch (currentTestMode) {
    case MODE_NONE:
      if (usingRangingLayer) DW1000Ranging.loop();
      break;
...
void parseCommand(String cmd) {
  ...
  if (command == "IDLE_TEST") startIdleTest();
    ...
    txInterval = (arg1 > 0) ? arg1 : 50;
    startTxTest();
  } else if (command == "RANGING_TEST") {
    if (arg1 > 0) startRangingTest(arg1);
    else Serial.println("Usage: RANGING_TEST <count>");
  } else if (command == "SLEEP_CYCLE") {
    sleepDuration = (arg1 > 0) ? arg1 : 5000;
    startSleepTest();
...
void applyChannelConfiguration() {
  ...
  // Configure for ranging mode - UPDATED to 16MHz PRF, 6800kbps, 64 preamble
...
void updateOledDisplay() {
  ...
  switch (currentTestMode) {
    case MODE_NONE:
      snprintf(line2, 20, "CH: %d", channel);
      updateOLED("Ready", line2);
      break;
    case MODE_IDLE_TEST: updateOLED("IDLE_TEST", "Min Power Mode"); break;
    case MODE_RX_TEST: updateOLED("RX_TEST", "RX Active"); break;
    case MODE_TX_TEST:
      snprintf(line2, 20, "TX Every %lums", txInterval);
      updateOLED("TX_TEST", line2);
      break;
    case MODE_RANGING_TEST:
      snprintf(line2, 20, "%d/%d ranges", rangingTestCurrentCount, rangingTestTargetCount);
      updateOLED("RANGING_TEST", line2);
      break;
    case MODE_SLEEP_CYCLE: updateOLED("SLEEP_CYCLE", inDeepSleep ? "Deep Sleep" : "Complete"); break;
    case MODE_AUTO_SEQUENCE:
      snprintf(line2, 20, "CH: %d AUTO", channel);
      updateOLED("AUTO_SEQ", line2);
      break;
  }
}
